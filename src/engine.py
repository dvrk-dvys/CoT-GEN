    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch, self.config.epoch_size)):
            ...
            if score > best_score:
                ...
                print(f'MODEL SAVED at {current_time}: {save_name}')
                sys.stdout.flush()
                print(save_name)
...
class ThorTrainer:
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch, self.config.epoch_size)):
            ...
            if score > best_score:
                ...
                self.logger.info('MODEL SAVED:', save_name)
    ...
    class ThorTrainer:
        ...
        def report_score(self, mode='valid'):
            ...
            res['Acc_SA'] = accuracy_score(self.golds['total'], self.preds['total'])
            res['F1_SA'] = f1_score(self.golds['total'], self.preds['total'], labels=[0, 1, 2], average='macro')
            res['F1_ESA'] = f1_score(self.golds['explicits'], self.preds['explicits'], labels=[0, 1, 2],
                                     average='macro')
            res['F1_ISA'] = f1_score(self.golds['implicits'], self.preds['implicits'], labels=[0, 1, 2],
                                     average='macro')
            res['default'] = res['F1_SA']
            res['mode'] = mode
            for k, v in res.items():
