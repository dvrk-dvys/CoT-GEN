...
class ThorTrainer:
        ...
        for epoch in tqdm(range(self.start_epoch, self.config.epochs),
            ...
            if score > best_score:
                ...
                mlflow.log_metrics({
                    ...
                    "Acc_SA": result.get("Acc_SA", 0),
                    "F1_SA": result.get("F1_SA", 0),
                    "F1_ESA": result.get("F1_ESA", 0),
                    "F1_ISA": result.get("F1_ISA", 0),
    ...
    def evaluate_step(self, dataLoader=None, mode='valid'):
        ...
        mlflow.log_metrics({
            "valid_accuracy": result.get("Acc_SA", 0),
            "valid_f1_score": result.get("F1_SA", 0),
            "Acc_SA": result.get("Acc_SA", 0),
            "F1_SA": result.get("F1_SA", 0),
            "F1_ESA": result.get("F1_ESA", 0),
            "F1_ISA": result.get("F1_ISA", 0),
            "Avg_Cosine_Similarity": result.get("Avg_Cosine_Similarity", 0),
            "valid_composite_score": result.get("composite_score", 0)
        }, step=self.global_epoch)
    ...
    def final_evaluate(self, epoch=0):
        ...
        mlflow.log_metrics({
            ...
            "Final_F1_ESA": res.get("F1_ESA", 0),
            "Final_F1_ISA": res.get("F1_ISA", 0),
